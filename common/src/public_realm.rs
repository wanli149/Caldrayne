...
pub const PUBLIC_REALM_ARTIFACT_IDENTITY_SCHEME: &str = "caldrayne-public-realm-sha256-v1";
pub const PUBLIC_REALM_ASSET_SPECIFIER: &str = "veldr.public_realm";

#[derive(Clone, Copy, Debug, Deserialize, Serialize, PartialEq, Eq, Default)]
#[serde(rename_all = "snake_case")]
pub enum PublicRealmSourceKind {
    #[default]
    Bundled,
    LauncherManaged,
    BootstrapManaged,
...
#[serde(rename_all = "snake_case")]
pub enum BundledPublicRealmTargetKind {
    Missing,
    LocalhostOrLoopback,
...
impl BundledPublicRealmTargetKind {
    pub const fn as_str(self) -> &'static str {
        match self {
...
#[derive(Clone, Copy, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
pub enum BundledPublicRealmTransportKind {
    DirectTcp,
    DirectQuic,
    SrvLookup,
}

impl BundledPublicRealmTransportKind {
    pub const fn as_str(self) -> &'static str {
        match self {
...
#[derive(Clone, Copy, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
pub enum BundledPublicRealmAuthMode {
    NoExternalAuth,
    ExternalProvider,
}

impl BundledPublicRealmAuthMode {
    pub const fn as_str(self) -> &'static str {
        match self {
...
pub struct PublicRealm {
    ...
    #[serde(default)]
    pub source_kind: PublicRealmSourceKind,
}

impl PublicRealm {
    pub fn load() -> Self {
        Ron::<Self>::load_expect_cloned(PUBLIC_REALM_ASSET_SPECIFIER).into_inner()
    }

    pub fn try_load() -> Result<Self, AssetError> {
        Ron::<Self>::load_cloned(PUBLIC_REALM_ASSET_SPECIFIER).map(|entry| entry.into_inner())
    }
    ...
    pub fn artifact_identity(&self) -> String {
        ...
        update_field(
            &mut hasher,
            "scheme",
            PUBLIC_REALM_ARTIFACT_IDENTITY_SCHEME,
        );
    ...
    pub fn target_kind(&self) -> BundledPublicRealmTargetKind {
        let server_address = self.server_address.trim();
        if server_address.is_empty() {
            return BundledPublicRealmTargetKind::Missing;
        }
        ...
        if host.eq_ignore_ascii_case("localhost") {
            BundledPublicRealmTargetKind::LocalhostOrLoopback
        } else {
            BundledPublicRealmTargetKind::NamedHostCandidate
        }
    }

    pub fn transport_kind(&self) -> BundledPublicRealmTransportKind {
        if self.use_srv {
            BundledPublicRealmTransportKind::SrvLookup
        } else if self.use_quic {
            BundledPublicRealmTransportKind::DirectQuic
        } else {
            BundledPublicRealmTransportKind::DirectTcp
        }
    }

    pub fn auth_mode(&self) -> BundledPublicRealmAuthMode {
        if self.auth_server.is_some() {
            BundledPublicRealmAuthMode::ExternalProvider
        } else {
            BundledPublicRealmAuthMode::NoExternalAuth
        }
    }
    ...
    pub fn non_local_cutover_gap_reasons(&self) -> Vec<&'static str> {
        ...
        match self.target_kind() {
            BundledPublicRealmTargetKind::Missing => {
            ...
            BundledPublicRealmTargetKind::NamedHostCandidate
            | BundledPublicRealmTargetKind::PublicIpCandidate => {},
        }
    ...
    pub fn posture(&self) -> BundledPublicRealmPosture {
        ...
        BundledPublicRealmPosture {
            ...
            rollout_readiness_scope: "non-local Public cutover still requires both a non-local \
                                      target candidate in public_realm.server_address and an \
                                      exact external auth pin in public_realm.auth_server; \
...
#[derive(Clone, Debug, Deserialize, Serialize, PartialEq, Eq)]
pub struct BundledPublicRealmPosture {
    pub source_kind: PublicRealmSourceKind,
    pub display_name: String,
    pub server_address_configured: bool,
    pub server_address: String,
    pub artifact_identity: String,
    pub target_kind: BundledPublicRealmTargetKind,
    pub target_is_non_local_candidate: bool,
    pub transport_kind: BundledPublicRealmTransportKind,
    pub use_srv: bool,
    pub use_quic: bool,
    pub validate_tls: bool,
    pub auth_mode: BundledPublicRealmAuthMode,
    pub auth_server: Option<String>,
...
fn bundled_target_kind_for_ip(ip: IpAddr) -> BundledPublicRealmTargetKind {
    match ip {
        IpAddr::V4(ip) if ip.is_loopback() => BundledPublicRealmTargetKind::LocalhostOrLoopback,
        ...
        IpAddr::V6(ip) if ip.is_loopback() => BundledPublicRealmTargetKind::LocalhostOrLoopback,
        ...
        IpAddr::V4(_) | IpAddr::V6(_) => BundledPublicRealmTargetKind::PublicIpCandidate,
    }
}
...
mod tests {
    ...
    fn public_realm_source_kind_defaults_to_bundled_when_omitted() {
        ...
        .expect("public realm should deserialize");
    ...
    fn posture_flags_private_ip_without_auth_as_transitional() {
        let posture = public_realm("192.168.1.8:14004", None).posture();
        ...
        assert_eq!(posture.auth_mode, BundledPublicRealmAuthMode::NoExternalAuth);
    ...
    fn posture_identifies_named_host_with_external_auth_as_non_local_candidate() {
        ...
        assert_eq!(posture.auth_mode, BundledPublicRealmAuthMode::ExternalProvider);
        assert!(posture.non_local_cutover_ready);
    ...
    fn posture_tracks_transport_flags() {
        let posture = public_realm_with_transport(
            "prod.realm.example",
            Some("https://auth.realm.example"),
            true,
            true,
    ...
    fn artifact_identity_changes_when_auth_pin_changes() {
        let baseline = public_realm("prod.realm.example:14004", Some("https://auth-a.example"))
            .artifact_identity();
        let changed = public_realm("prod.realm.example:14004", Some("https://auth-b.example"))
            .artifact_identity();
