...
const CALDRAYNE_USERDATA_ENV: &str = "CALDRAYNE_USERDATA";
...
/// Determines common user data directory used by frontends.
///
/// The first specified in this list is used.
///   1. The CALDRAYNE_USERDATA runtime environment variable
///   2. The CALDRAYNE_USERDATA_STRATEGY compile time environment variable
///   3. The CARGO_WORKSPACE_DIR/userdata compile time environment variable
///      defined in .cargo/config.toml
///
/// ### `CALDRAYNE_USERDATA_STRATEGY` environment variable
///
...
pub fn userdata_dir() -> PathBuf {
    // 1. The CALDRAYNE_USERDATA runtime environment variable
    std::env::var_os(CALDRAYNE_USERDATA_ENV)
        .map(PathBuf::from)
        // 2. The CALDRAYNE_USERDATA_STRATEGY compile time environment variable
        .or_else(|| match option_env!("CALDRAYNE_USERDATA_STRATEGY") {
            // "system" => system specific project data directory
            Some(s) if s.eq_ignore_ascii_case("system") => Some(directories_next::ProjectDirs::from("io", "Caldrayne", "Caldrayne")
            ...
            Some(s) => {
                warn!(
                    "Compiled with an invalid CALDRAYNE_USERDATA_STRATEGY: \"{}\". \
        ...
        .unwrap_or_else(|| {
            ...
            if path.exists() && exe_path.starts_with(&path) {
                ...
                warn!(
                    ...
                    and was not compiled with CALDRAYNE_USERDATA_STRATEGY set to \"system\" or \"executable\". \
