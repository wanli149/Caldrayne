...
impl FileSystem {
    pub fn new() -> io::Result<Self> {
        ...
        if !canary.starts_with("VELOREN_CANARY_MAGIC") {
            panic!("Canary asset `canary.canary` was present but did not contain the expected data. This strongly suggests Git LFS (Large File Storage) is not set up correctly. Make sure Git LFS is installed and run `git lfs install --local` followed by `git lfs pull`.");
        }
