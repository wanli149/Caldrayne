...
hud-press_key_to_respawn = 按下 { $key }，在你最后造访的篝火处重生。
hud-tutorial_btn = 教程
hud-tutorial_click_here = 按下 [ { $key } ] 释放鼠标光标，然后点击这个按钮！
hud-tutorial_elements = 制作
hud-temp_quest_headline = 旅行者，欢迎来到这里！
hud-temp_quest_text =
    若想开启旅程，不妨先在村子里四处看看，顺手收集一些物资。

    旅途中看到需要的东西，尽管取用便是。

    屏幕右下角可以找到不少常用功能，比如背包、制作和地图。

    通过制作界面，你可以打造护甲、武器、食物等等。

    村庄周围有不少野生动物，它们会掉落皮料，是你前期制作防具的重要材料。

    等你准备得差不多了，就去挑战地图上的标记地点吧，那里往往藏着更好的装备！
hud-spell = 法术
hud-diary = 技能书
...
hud-camera_clamp_indicator = 锁定视角已启用。按下 { $key } 可关闭。
...
hud-mine-needs_unhandled_case = 需要特定工具
hud-talk = 交谈
hud-trade = 交易
hud-mount = 攀爬
hud-sit = 坐下
...
hud-init-stage-server-worldsim-erosion = { "[" }{ -server }]: 地形侵蚀中 { $percentage }%
hud-init-stage-server-worldciv-civcreate = { "[" }{ -server }]: 已生成 { $generated } / { $total } 个文明
hud-init-stage-server-worldciv-site = { "[" }{ -server }]: 正在生成聚落...
hud-init-stage-server-economysim = { "[" }{ -server }]: 正在模拟经济...
hud-init-stage-server-spotgen = { "[" }{ -server }]: 生成场所...
hud-init-stage-server-starting = { "[" }{ -server }]: 启动服务器...
hud-init-stage-multiplayer = 正在启动多人模式
...
hud-init-stage-client-starting-client = { "[" }{ -client }]: 正在准备客户端...
...
hud-downed_recieving_help = 正在接受救援。
hud-steal = 窃取
hud-dig = 挖掘
