...
command-gm-desc = Toggle GM work mode for admin camera access and freefly.
...
command-wiki-desc = Open the project page or search the repository
...
command-gm-status-on = GM mode is currently enabled.
command-gm-status-off = GM mode is currently disabled.
command-gm-enabled = GM mode enabled. Admin camera access and freefly are now available.
command-gm-disabled = GM mode disabled. Returned to player camera rules.
