...
pub enum ClientChatCommand {
    ...
    /// located.
    Waypoint,
    /// Opens the Caldrayne project page in a browser
    Wiki,
}
...
///
/// With no arguments, opens the Caldrayne project homepage.
/// With arguments, performs a repository search for the specified terms.
...
fn handle_wiki(
    ...
    let url = if args.is_empty() {
        "https://github.com/wanli149/Caldrayne".to_string()
    } else {
        let query_string = args.join("+");

        format!("https://github.com/wanli149/Caldrayne/search?q={query_string}")
    };
