use crate::{
    ...
    frame::InitFrame,
    types::{
        Pid, STREAM_ID_OFFSET1, STREAM_ID_OFFSET2, Sid, VELDR_MAGIC_NUMBER,
        VELDR_NETWORK_VERSION,
    },
};
    ...
    async fn initialize(
        ...
        match sink.recv().await? {
            InitFrame::Handshake {
                ...
                if magic_number != VELDR_MAGIC_NUMBER {
                ...
                } else if version[0] != VELDR_NETWORK_VERSION[0]
                    || version[1] != VELDR_NETWORK_VERSION[1]
                {
                    ...
                    drain
                        .send(InitFrame::Raw(
                            format!(
                                "{} Our Version: {:?}\nYour Version: {:?}\nClosing the connection",
                                WRONG_VERSION, VELDR_NETWORK_VERSION, version,
                            )
                            .as_bytes()
                            .to_vec(),
                        ))
...
mod tests {
    ...
    async fn handshake_wrong_magic_number() {
        ...
        assert_eq!(
            r1.unwrap(),
            Err(InitProtocolError::WrongMagicNumber(*b"woops"))
        );
