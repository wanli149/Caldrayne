...
use vek::*;
use veldr_client::{Client, ClientType, addr::ConnectionArgs};
...
fn run_client(
    ...
    let mut tick = |client: &mut Client| -> Result<(), veldr_client::Error> {
        clock.tick();
...
fn position(index: u32, opt: Opt) -> Vec3<f32> {
    ...
    let movement_offset: Vec2<_> = if opt.movement {
        // blocks per second
        const SPEED: f32 = 9.0; // typical super fast veldr walking speed
