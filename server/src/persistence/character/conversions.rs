...
use common::{
    ...
    comp::{
        ...
        Waypoint, body,
        inventory::{
            item::{Item as InternalItem, MaterialStatManifest, tool::AbilityMap},
...
fn get_mutable_item<'a, 'b, T>(
    ...
    inventory: &'b mut T,
    get_mut_item: &'a impl Fn(&'b mut T, &str) -> Option<&'b mut InternalItem>,
) -> Result<&'a mut InternalItem, PersistenceError>
where
    'b: 'a,
{
    ...
    if inventory_items[index].position.contains("component_") {
        ...
        get_mut_item(inventory, &inventory_items[index].position).ok_or_else(|| {
            PersistenceError::ConversionError(format!(
                "Unable to retrieve parent veldr item {parent_id} of component from inventory."
            ))
        })
    }
}
    ...
    for (i, db_item) in inventory_items.iter().enumerate() {
        ...
        if db_item.parent_container_item_id == inventory_container_id {
            ...
            get_mutable_item(
                ...
                &|(inv, f_i): &mut (&mut Inventory, &mut HashMap<String, InternalItem>), s| {
...
pub fn convert_overflow_items_from_database_items(
    overflow_items_container_id: i64,
    database_items: &[Item],
) -> Result<Vec<InternalItem>, PersistenceError> {
