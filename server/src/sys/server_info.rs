...
impl<'a> System<'a> for Sys {
    ...
    fn run(
        ...
        (tick, settings, identity, sender, players, clients): Self::SystemData,
    ) {
            ...
            && tick.0 % INFO_SEND_INTERVAL == 0
        {
            let count = (&players, &clients)
                ...
                .try_into()
                .unwrap_or(u16::MAX);
            if let Err(e) = sender.send(build_query_server_info(&settings, &identity, count)) {
